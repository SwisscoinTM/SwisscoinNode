...
#if defined(NDEBUG)
# error "SWISSCOIN cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 60 * 60 * 24; // 60 seconds, 60 minutes, 24 hours
...
{
    int64_t nSubsidy = 40000000 * COIN;
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProofOfWork() && nHeight > 50 && nHeight < 303255)
        return DoS(100, error("AcceptBlock() : reject proof-of-work at height %d", nHeight));

    if (IsProofOfStake() && nHeight <= Params().LastPOWBlock() && nHeight > 303255)
    ...
    if (nBits != GetNextTargetRequired(pindexPrev, IsProofOfStake()) && nHeight > Params().LastPOWBlock())
...
{
    RenameThread("swisscoin-loadblk");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (pfrom->nVersion < MIN_PEER_PROTO_VERSION && pfrom->addrName != "89.163.230.71:16714")
        {
            // disconnect from peers older than this proto version
            LogPrintf("partner %s using obsolete version %i; disconnecting. PLEASE UPDATE\n", pfrom->addr.ToString(), pfrom->nVersion);
